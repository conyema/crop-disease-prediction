...
class ModelSelector:
    ...
    def select_model(self):
        ...
        if model_name == "resnet18":
            ...
            raise ValueError(
                f"Model {self.model_name} is not supported. Choose from 'resnet18','resnet34', 'resnet50', 'efficientnet_b0', 'efficientnet_b1', 'efficientnet_b2','efficientnet_b3', 'efficientnet_b4' ."
            )
...
def load_model(file_path, config):
    ...
    try:
        ...
        loaded_model = torch.load(file_path, map_location=torch.device(device))
        ...
        if isinstance(loaded_model, dict):
            ...
            new_state_dict = {
                k.replace("_orig_mod.", ""): v for k, v in loaded_model.items()
            }
