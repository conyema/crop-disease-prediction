...
def get_image_options(path):
    ...
    image_files = [
        f for f in os.listdir(path) if f.split(".")[-1].lower() in image_extensions
    ]
    return image_files
...
def display_prediction(prediction, confidence):
    ...
    with col2:
        ...
        if "healthy" in prediction.lower():
            st.markdown(
                f"<p class='prediction-green'>{prediction.replace('_', ' ').title()}</p>",
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                f"<p class='prediction-red'>{prediction.replace('_', ' ').title()}</p>",
                unsafe_allow_html=True,
            )
    ...
    with col3:
        ...
        if confidence > 85:
            st.markdown(
                f"<p class='confidence-green'>{confidence}%</p>", unsafe_allow_html=True
            )
        elif 65 <= confidence <= 85:
            st.markdown(
                f"<p class='confidence-yellow'>{confidence}%</p>",
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                f"<p class='confidence-red'>{confidence}%</p>", unsafe_allow_html=True
            )
...
def display_actionable_insights(selected_crop, predicted_disease):
    ...
    if selected_crop in data and predicted_disease in data[selected_crop]:
        ...
        st.warning(
            "No information available for the selected crop and predicted disease."
        )
...
def display_guidelines(crop_name):
    ...
    if "strawberry" in crop_name_lower:
        st.markdown(
            f"<h4>General guidelines for Growing Strawberry:</h4>",
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            f"<h4>General guidelines for Growing {crop_name}:</h4>",
            unsafe_allow_html=True,
        )
...
def main():
    ...
    crop_name = st.selectbox(
        "Select Crop for Disease Identification", crop_names, index=0
    )
    ...
    with st.expander("Information about the disease Identification"):
        st.write(
            f"**This model is trained to predict these diseases in {crop_name} :**"
        )
    ...
    st.text(
        "ℹ️ Focus on diseased leaves/fruit, keep the area of interest at the center of the image for better results"
    )
        ...
        with col2:
            ...
            if len(image_options) > 0:
                selected_image = st.selectbox(
                    "Select an Image", image_options, index=None
                )
    ...
    if st.button("Predict"):
        ...
        prediction, prediction_probability = predict_single_image(
            image=image, model=model, config=config, device=device
        )
    ...
    if prediction is not None:
        with st.container(border=True):
            display_actionable_insights(
                selected_crop=crop_name_lower, predicted_disease=prediction
            )
